  ...
  try {
    ...
    if (search) {
      const res = await fetch(
        `https://api.willyweather.com.au/v2/${willyWeatherApiKey}/search.json?query=${encodeURIComponent(search)}`,
      ...
      console.log("WillyWeather search response:", JSON.stringify(data, null, 2));
      // The search API can return results in different structures.
      // This handles both { locations: { location: [...] } } and a direct array.
      return NextResponse.json(data.locations?.location || data.location || data || []);
    }

    // Handle fetching detailed info for a specific location
    if (locationId && type === 'info') {
      const res = await fetch(
        `https://api.willyweather.com.au/v2/${willyWeatherApiKey}/locations/${locationId}/weather.json?observational=true&forecasts=wind`,
